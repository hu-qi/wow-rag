...
> 前端页面当前是无构建的静态 HTML，Vue、Tailwind、Marked、字体和样式会从 CDN 加载。首次打开页面时需要能够访问这些 CDN；如果处于离线或网络受限环境，页面可能出现样式缺失、交互不可用或 Markdown 渲染失败等问题。
...
- 本项目的前端页面chat.html，直接双击打开。在文本框中输入问题，然后点击发送按钮即可体验流式对话。该页面依赖在线 CDN 资源，适合快速体验；如需离线或产品化部署，建议后续再引入本地依赖管理或前端构建流程。
